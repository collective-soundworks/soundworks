...
import { kSharedStatePromiseStore } from '../../src/common/SharedState.js';
  ...
  describe(`## MISC`, () => {
    it(`check consistency of PromiseStore in SharedState`, async () => {
      ...
      { // update promise
        ...
        assert.equal(state[kSharedStatePromiseStore].store.size, 1);
        assert.equal(attached[kSharedStatePromiseStore].store.size, 0);

        await promise;

        assert.equal(state[kSharedStatePromiseStore].store.size, 0);
        assert.equal(attached[kSharedStatePromiseStore].store.size, 0);
      }
      ...
      { // update promise
        ...
        assert.equal(state[kSharedStatePromiseStore].store.size, 0);
        assert.equal(attached[kSharedStatePromiseStore].store.size, 1);
    ...
    it(`stress test`, async () => {
      ...
      for (let i = 0; i < 1e4; i++) {
        ...
        assert.equal(state[kSharedStatePromiseStore].store.size, i + 1);
        promises.push(promise);
      }

      assert.equal(state[kSharedStatePromiseStore].store.size, 1e4);
