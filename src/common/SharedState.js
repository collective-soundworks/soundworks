...
${JSON.stringify(initValues, null, 2)}`);
    ...
    client.transport.addListener(`${UPDATE_RESPONSE}-${id}-${this.remoteId}`, (reqId, updates) => {
      const updated = this._commit(updates, true, true);
    ...
    client.transport.addListener(`${UPDATE_ABORT}-${id}-${this.remoteId}`, (reqId, updates) => {
      const updated = this._commit(updates, false, true);
      resolveRequest(reqId, updated);
    });
    ...
    client.transport.addListener(`${UPDATE_NOTIFICATION}-${id}-${this.remoteId}`, (updates) => {
      ...
      this._commit(updates, true, false);
    });
  ...
  /** @private */
  _commit(updates, propagate = true, initiator = false) {
    const updated = {};

    for (let name in updates) {
      const { immediate, filterChange, event } = this._parameters.getSchema(name);
      const [value, changed] = this._parameters.set(name, updates[name]);
    ...
    if (propagate && Object.keys(updated).length > 0) {
  ...
  async set(updates) {
    ...
    for (let name in updates) {
      ...
      // @note: general idea...
      // if immediate=true && filterChange=true
      //  - call listeners if value changed
      //  - else go through normal server path
      //  - retrigger only if response from server is different from current value
      // if immediate=true && filterChange=false (...same for event=true)
      //  - call listeners with value regarless it changed
      //  - go through normal server path
      //  - if the node is initiator of the update (UPDATE_RESPONSE)
      //    - we need to (re-)check to prevent trigger listeners a second time
      ...
      if (immediate) {
        ...
        if (changed || filterChange === false) {
          immediates[name] = updates[name];
          propagateNow = true;
        }
      }
    }

    // console.log(immediates, propagateNow);
    if (propagateNow) {
      this._subscriptions.forEach(listener => listener(immediates));
    }
