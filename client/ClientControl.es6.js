...
class Information {
  constructor(information, view = null) {
...
class ClientControl extends ClientModule {
  constructor(options = {}) {
    ...
    client.receive('control:init', (parameters, informations, commands) => {
      if (view) {
      ...
      for (let key of Object.keys(informations))
        this.informations[key] = new Information(informations[key], view);
    ...
    // listen to information changes
    client.receive('control:information', (name, val) => {
      var information = this.informations[name];

      if (information) {
        information.set(val);
        this.emit('control:information', name, val);
      } else
        console.log('received unknown information parameter: ', name);
    });
  }
